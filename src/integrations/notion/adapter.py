...
from src.domain.models import ContentRecord, NotionPage, ReportData
from src.domain.enums import Category, Difficulty
from src.utils.logger import get_logger
from src.utils.datetime_utils import format_datetime
from config.settings import settings
from config.topics import get_category_name, CATEGORIES
...
class NotionAdapter:
    ...
    async def create_content_page(
        ...
        category_value = content.category.value if isinstance(content.category, Category) else content.category
        category_name = get_category_name(category_value, settings.language)

        # Get difficulty display name
        difficulty_value = content.difficulty.value if isinstance(content.difficulty, Difficulty) else content.difficulty
        difficulty_name = content.difficulty.korean if hasattr(content.difficulty, 'korean') else difficulty_value
        ...
        try:
            properties = {
                ...
                "난이도": {
                    "select": {"name": difficulty_name}
                },
                "태그": {
                    "multi_select": [
                        {"name": tag} for tag in content.tags[:10]
                    ]
                },
                "작성일": {
                    "date": {
                        "start": content.created_at.strftime("%Y-%m-%d") if content.created_at else datetime.now().strftime("%Y-%m-%d")
                    }
                },
                "작성자": {
            ...
            children = [
                {
                    "type": "callout",
                    "callout": {
                        "rich_text": [{"type": "text", "text": {"content": content.summary[:2000]}}],
    ...
    async def create_report_page(
        ...
        report_type_ko = "주간" if report.report_type.value == "weekly" else "월간"
        ...
        try:
            properties = {
                ...
                "작성일": {
                    "date": {
                        "start": report.generated_at.strftime("%Y-%m-%d") if report.generated_at else datetime.now().strftime("%Y-%m-%d")
                    }
                },
                "작성자": {
                    "rich_text": [{"text": {"content": "Daily-Bot"}}]
                },
                "상태": {
                    "select": {"name": "발행됨"}
                }
            ...
            children = [
                ...
                },
                {
                    ...
                    "bulleted_list_item": {
                        "rich_text": [{"type": "text", "text": {"content": f"기간: {format_datetime(report.period_start, False)} ~ {format_datetime(report.period_end, False)}"}}]
                    }
                },
                {
                    "type": "bulleted_list_item",
                    "bulleted_list_item": {
                        "rich_text": [{"type": "text", "text": {"content": f"총 발송: {report.total_count}건"}}]
                    }
                },
                {
                    "type": "bulleted_list_item",
                    "bulleted_list_item": {
                        "rich_text": [{"type": "text", "text": {"content": f"성공: {report.success_count}건"}}]
                    }
                },
                {
                    "type": "bulleted_list_item",
                    "bulleted_list_item": {
                        "rich_text": [{"type": "text", "text": {"content": f"실패: {report.failed_count}건"}}]
                    }
                },
                {
                    "type": "bulleted_list_item",
                    "bulleted_list_item": {
                        "rich_text": [{"type": "text", "text": {"content": f"재시도: {report.retry_count}건"}}]
                    }
                },
            ]
            ...
            if report.avg_duration_ms:
                children.append({
                    "type": "bulleted_list_item",
                    "bulleted_list_item": {
                        "rich_text": [{"type": "text", "text": {"content": f"평균 실행시간: {report.avg_duration_ms:.0f}ms (최소: {report.min_duration_ms}ms, 최대: {report.max_duration_ms}ms)"}}]
                    }
                })
            ...
            if report.category_distribution:
                ...
                for cat, count in sorted(
                    ...
                    children.append({
                        "type": "bulleted_list_item",
                        "bulleted_list_item": {
                            "rich_text": [{"type": "text", "text": {"content": f"{cat_name}: {count}건"}}]
                        }
                    })
        ...
        while i < len(lines):
            ...
            # Numbered list
                ...
                blocks.append({
                    ...
                    "numbered_list_item": {
                        "rich_text": self._parse_inline_formatting(line[2:].strip())
                    }
                })
            # Quote
            elif line.startswith("> "):
